...
export default function ExamPage() {
  ...
  const handleOptionSelect = (option: "A" | "B" | "C" | "D") => {
    ...
    setAnswers((prev) => ({
      ...prev,
      [currentQuestion._id]: { questionId: currentQuestion._id, selectedAnswer: option, timeSpent },
    }));
    setSelectedOption(option);
  };

  // Next question (option can be null if skipped)
  const handleNextQuestion = (option: "A" | "B" | "C" | "D" | null) => {
    ...
    setSelectedOption(answers[questions[currentIndex + 1]?._id]?.selectedAnswer || null);
  ...
  const goToResultPage = () => {
    router.push("/exam/result"); // redirect to result page
  };

  const formatTime = (seconds: number) =>
    `${Math.floor(seconds / 60)}:${(seconds % 60).toString().padStart(2, "0")}`;

  if (isLoading) return <p className="text-center mt-10">Loading questions...</p>;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <div className="max-w-xl w-full bg-white rounded-xl shadow-md p-6 md:p-8 text-center">
          ...
          <p className="mb-6">You have completed all questions for this step.</p>
          <button
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="max-w-4xl w-full bg-white rounded-xl shadow-md p-6 md:p-8">
        ...
        <div className="flex justify-between mb-4">
          <div>
            <h1 className="text-lg font-semibold">Step {step} - Digital Competency Exam</h1>
            <p className="text-sm text-gray-600">{currentQuestion.competency}</p>
          </div>
          <div className={`text-lg font-bold ${timeLeft < 10 ? "text-red-500" : "text-blue-600"}`}>
            ⏱️ {formatTime(timeLeft)}
          </div>
        </div>

        {/* Question */}
        <h2 className="text-xl md:text-2xl font-bold mb-6">{currentQuestion.questionText}</h2>
              ...
              <div className="flex items-center">
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-3 ${
                    selectedOption === opt ? "bg-blue-500 border-blue-500 text-white" : "border-gray-300 text-gray-600"
                  }`}
                >
                  {opt}
                </div>
